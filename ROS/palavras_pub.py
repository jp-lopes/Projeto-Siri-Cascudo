...
def transcrever_audio_em_tempo_real():
    pub = rospy.Publisher('palavras', String, queue_size=10)
    rospy.init_node('reconhecimento_palavras')
    rate = rospy.Rate(20)
    while not rospy.is_shutdown():
        ...
        try:
            with sr.Microphone() as source:
                ...
                rospy.loginfo("Pronto! Fale agora... (pressione 'q' para parar)")
                ...
                while not stop_event.is_set():
                    try:
                        ...
                        if "dinheiro" in texto_lower:
                            rospy.loginfo(f"Comando detectado (dinheiro): {texto_transcrito}")
                            ...
                            rospy.loginfo(f"Comando detectado (siri cascudo): {texto_transcrito}")
                            pub.publish(texto_transcrito)
                            ...
                            rospy.loginfo(f"Comando detectado (bob esponja): {texto_transcrito}")
                            pub.publish(texto_transcrito)
                            ...
                            rospy.loginfo(f"Comando detectado (lula molusco): {texto_transcrito}")
                            pub.publish(texto_transcrito)
                            ...
                            rospy.loginfo(f"Comando detectado (nematelminos): {texto_transcrito}")
                            pub.publish(texto_transcrito)
                            ...
                            rospy.loginfo(f"Nenhum omando detectado: {texto_transcrito}")
                            pub.publish(texto_transcrito)
                        ...
                        rospy.loginfo(f"Erro na requisição ao serviço de reconhecimento; {e}")
